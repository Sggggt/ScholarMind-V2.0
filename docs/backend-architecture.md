...
### 10.1 进程组杀灭（Process Group Kill）

**问题**：Aider 子进程内部会派生 git 子进程。超时后 `process.kill()` 只杀主进程，git 子进程变为僵尸（zombie），阻塞后续运行。

**方案**：所有子进程使用 `start_new_session=True` 创建独立进程组，超时/取消时用 `os.killpg(pgid, SIGKILL)` 杀灭整棵进程树。
...
```
              ...
              ▼        ▼        ▼
         ┌────────┐┌────────┐┌────────┐
         │git add ││git     ││python  │  同一 pgid
         │        ││commit  ││child   │
         └────────┘└────────┘└────────┘

  os.killpg(PGID, SIGKILL) → 杀灭整棵树
```

**涉及文件**：`backend/modules/async_subprocess.py`

- async 路径：`asyncio.create_subprocess_exec(..., start_new_session=True)` + `os.killpg`
- threaded 路径：`subprocess.Popen(..., start_new_session=True)` + `_kill_process_tree()`

### 10.2 超时机制修复（deadline 模式）

**问题**：`TaskStateMachine.run_interruptible()` 用 `asyncio.shield(task)` + `poll_interval=0.5s` 轮询，每轮 `TimeoutError` 都 `continue`，导致 `asyncio.wait_for(communicate, timeout=600)` 的总超时永远不触发。

**方案**：`run_interruptible` 新增 `deadline` 参数（单调时钟绝对时间），每轮 poll 检查是否超过 deadline，超时则 cancel task 并抛出 `asyncio.TimeoutError`。`run_subprocess` 在有 `state` 时把 timeout 转为 deadline 传入。

```
  run_subprocess(timeout=600, state=sm)
    → deadline = loop.time() + 600
    → sm.run_interruptible(communicate, deadline=deadline)
      → 每轮 poll:
          if now >= deadline → cancel + TimeoutError
          else → wait min(poll_interval, remaining)
```

**涉及文件**：`backend/pipeline/state.py`、`backend/modules/async_subprocess.py`

### 10.3 Aider Tags Cache 优化

**问题**：Aider 的 `.aider.tags.cache.v4/` 目录（ctags 索引缓存，数百个小文件）存储在 Docker 9P volume 上，读写延迟极高，导致进程卡在 `p9_client_rpc` 等待状态。

**方案**：
- 每次 Aider 运行前 `_clean_aider_tags_cache()` 删除旧缓存
- 设置 `AIDER_HOME` 环境变量指向容器本地 `/tmp/` 路径，使 Aider 在本地文件系统而非 9P volume 上重建缓存

**涉及文件**：`backend/modules/aider_runner.py`

### 10.4 僵尸 Git 进程清理

**方案**：Aider preflight 阶段调用 `_reap_zombie_git_processes()`，扫描进程表中状态为 `Z`（zombie）且属于当前仓库路径的 git 进程，尝试 `SIGKILL` 清理。
...
### 11.1 AgentRuntime 服务

**新增** `backend/services/agent_runtime.py`，统一管理多代理树的状态广播：

- 维护 `active_cycle`、`root_agent`、`child_agents`、`recent_summary` 四维代理状态
- 通过 WebSocket 实时推送 `agent_tree`、`agent_event`、`agent_summary` 消息类型
- 支持模块级、阶段级、角色级的事件分类

### 11.2 M6 Agent Runner 增强

**涉及文件**：`backend/modules/m6_agent_runner.py`（+822 行）

- **本地实验执行**：集成 `local_env_setup.py` 管理实验虚拟环境
- **实验循环**：支持多轮实验执行（run_1, run_2, ...），自动重试失败实验
- **压缩反馈**：实验失败时压缩错误信息反馈给 coordinator
- **回卷机制**：coordinator 可决定回卷到 M4 重新规划代码
- **LLM 模拟模式**：`llm_simulation_enabled` 选项支持不跑真实实验

### 11.3 M4 代码生成增强

**涉及文件**：`backend/modules/m4_code_gen.py`（+255 行）

- **静态门禁**：验证生成的代码包含 idea 关键词
- **LLM 全量重写 fallback**：Aider 失败后用 LLM 直接重写 experiment.py
- **Git 仓库管理**：自动初始化 git 仓库、提交基线代码
- **基线运行**：自动执行 run_0 获取基线指标

### 11.4 M5 实验设计改进

**涉及文件**：`backend/modules/m5_experiment_design.py`（+187 行）

- AI-Scientist 风格的实验设计 prompt
- 支持生成多个实验方案（最多 N runs）
- Aider 实现代码修改 + LLM fallback 双路径

### 11.5 本地实验环境

**新增** `backend/modules/local_env_setup.py`

- `ensure_local_experiment_env()`：在项目目录下创建 `.venv-experiment`
- 指纹校验：通过 `requirements.txt` 的 SHA256 判断是否需要重建 venv
- 自动安装依赖、升级 pip
- 状态持久化：`.scholarmind-local-env.json` 记录环境状态
